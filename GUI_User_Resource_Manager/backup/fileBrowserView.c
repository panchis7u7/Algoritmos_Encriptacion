  ...
  if (!window)
    {
      ...
      FileBrowserView* fbv = file_browser_view_new(GTK_WINDOW(window));

      g_signal_connect(fbv, "clicked", G_CALLBACK(filebrowser_up_clicked_cb), NULL);
      g_signal_connect(fbv, "activate", G_CALLBACK(filebrowser_view_activated_cb), NULL);
